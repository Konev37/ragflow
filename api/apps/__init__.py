...
def _load_user():
    ...
    # 1. Try JWT decoding if it looks like a JWT
    if auth_token.count('.') == 2:
        try:
            ...
            logging.debug(f"load_user: JWT decode failed (likely not a JWT): {e_jwt}")

    # 2. Try as API Token (Standard hex/uuid format)
    try:
        ...
        if objs:
            ...
            if user:
                ...
                g.user = user[0]
                return user[0]
            logging.warning(f"load_user: No valid user found for tenant_id={objs[0].tenant_id}")
    except Exception as e_api_token:
        logging.error(f"load_user: API token query failed: {e_api_token}")
